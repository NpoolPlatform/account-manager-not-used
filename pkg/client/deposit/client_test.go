...
var accountData = npool.Account{
	...
	ScannableAt:   uint32(time.Now().Unix()) + 100,
	Incoming:      "0",
	Outcoming:     "0",
}
...
var (
	accountInfo = npool.AccountReq{
		...
		ScannableAt:   &accountData.ScannableAt,
...
func createAccount(t *testing.T) {
	...
	if assert.Nil(t, err) {
		...
		accountData.ScannableAt = info.ScannableAt
...
func updateAccount(t *testing.T) {
	var err error
	tm := uint32(time.Now().Unix() + 1000)
	accountInfo.ScannableAt = &tm
...
func createAccounts(t *testing.T) {
	...
	for key := range accountDatas {
		Accounts = append(Accounts, &npool.AccountReq{
			...
			ScannableAt:   &accountDatas[key].ScannableAt,
